...
-- SKIPPED, NOT PASSING, and the reason is a defect outside this table.
--
-- Deleting this organization currently fails with:
--   ERROR: insert or update on table "audit_log" violates foreign key
--          constraint "audit_log_org_id_fkey"
--   Key (org_id)=(...) is not present in table "organizations"
--
-- audit_row_change() nulls a vanished org_id only `if tg_op = 'DELETE'`. The
-- membership cascade's `on delete set null` fires an UPDATE on bricks, whose
-- org is mid-delete, and the UPDATE path has no such guard. bricks is the
-- first table to have BOTH a transitive cascade from organizations and a
-- set-null from memberships, which is why nothing has hit this before.
--
-- The one-condition fix belongs in audit_row_change()'s own migration, which
-- is outside this session's scope (workflow rule 4). Skipped rather than
-- deleted so every CI run keeps saying so.
--
-- Un-skip by widening that guard to any tg_op. Verified diagnosis: test 24
-- above passes, i.e. deleting a membership works while the org survives.
select skip(
  'audit_row_change() nulls a vanished org_id on DELETE only; the membership '
  'cascade fires an UPDATE. Owed as a fix to that function.', 2
);
